...
RSpec.describe ParticipantsController, type: :controller do
  ...
  let(:participant) { FactoryBot.create(:participant, stage: 'approved', address_line1: '123 Main St', address_line2: 'Apt #123', city: 'Chicago', state: 'IL', zip: '12345', email: 'test@test.com', primary_phone: '123-456-7890', secondary_phone: '123-345-6789')}
  ...
  describe 'authorized access' do
    ...
    describe 'POST update' do
      describe 'with valid parameters' do
        it 'updates participant attributes' do
          post :update, params: { id: participant.id, participant: { first_name: 'Azazello'} }
        ...
        it 'updates participant relationship attributes' do
          ...
          post :update, params: { id: participant.id, participant: { first_name: 'Azazello', origin_relationships_attributes: { id: origin_relationship.id, category: 'Guardian'}} }
    ...
    describe 'GET enroll' do
      ...
      it 'copies data from existing active participants' do
        ...
        account.participants << other_participant
        account.save!
        get :enroll, params: { id: other_participant.id }
        expect(account.participants.last.address_line1).to eq account.participants.first.address_line1
      end
    ...
    describe 'GET consent' do
      it 'renders consent template' do
        get :consent, params: { id: participant.id }
    ...
    describe 'POST consent signature' do
      ...
      describe 'with valid params' do
        it 'transitions participant to consented state if consent is accepted' do
          post :consent_signature, params: { id: participant.id, consent_response: 'accept', consent_signature: { date: Date.today, consent_id: @adult_survey.id, proxy_name: 'Little My', proxy_relationship: 'Parent'} }
        ...
        it 'transitions participant to consent denied state if consent is not accepted' do
          post :consent_signature, params: { id: participant.id, consent_response: 'blah' }
      ...
      describe 'with invalid params' do
        it 'redirects to dashboard' do
          post :consent_signature, params: { id: participant.id, consent_response: 'accept', consent_signature: { proxy_name: 'hello'} }
          expect(response).to redirect_to(controller: :participants, action: :enroll, id: participant.id)
        end
  ...
  describe 'unauthorized access' do
    ...
    describe 'POST create' do
      it 'does not create a participant' do
        expect {
          post :create, params: { account_id: '999999' }
    ...
    describe 'POST update' do
      it 'redirects to dashboard' do
        post :update, params: { id: participant.id }
        expect(response).to redirect_to(controller: :accounts, action: :dashboard)
      end
    end
