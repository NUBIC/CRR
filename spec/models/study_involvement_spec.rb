...
RSpec.describe StudyInvolvement, type: :model do
  ...
  let(:study_involvement) { FactoryBot.create(:study_involvement, participant: participant, study: study) }
  ...
  describe 'validates end_date' do
    it 'should not be before start_date' do
      study_involvement = FactoryBot.build(:study_involvement, start_date: date, end_date: date - 1.days)
  ...
  describe 'validating participant enrollment' do
    it 'should not allow to enroll participant twice in the same study' do
      study_involvement = FactoryBot.create(:study_involvement, participant: participant, study: study)
    ...
    it 'should allow to create extended participant enrollment' do
      study_involvement = FactoryBot.create(:study_involvement, participant: participant, study: study)
      new_study_involvement = FactoryBot.create(:study_involvement, participant: participant, study: study, extended_release: true)
  ...
  describe 'scopes:' do
    it 'active scope should return qualifying study involvements' do
      date = Date.today
      si1 = FactoryBot.create(:study_involvement, start_date: date - 10.days, end_date: date + 1.days)
      si2 = FactoryBot.create(:study_involvement, start_date: date - 10.days, end_date: date)
      si3 = FactoryBot.create(:study_involvement, start_date: date - 20.days, end_date: date - 15.days)
      si4 = FactoryBot.create(:study_involvement, start_date: date + 1.days, end_date: date + 10.days)
      expect(StudyInvolvement.active).to match_array([si1, si2])
    end

    it 'warning scope returns qualifying study involvements' do
      date = Date.today
      si1 = FactoryBot.create(:study_involvement, start_date: date - 10.days, end_date: date + 10.days)
      si2 = FactoryBot.create(:study_involvement, start_date: date - 10.days, end_date: date + 10.days, warning_date: date)
      si3 = FactoryBot.create(:study_involvement, start_date: date - 10.days, end_date: date + 10.days, warning_date: date + 3.days)
      si4 = FactoryBot.create(:study_involvement, start_date: date - 10.days, end_date: date + 10.days, warning_date: date - 3.days)
      si5 = FactoryBot.create(:study_involvement, start_date: date - 10.days, end_date: date - 2.days, warning_date: date - 3.days)
      expect(StudyInvolvement.warning).to match_array([si2, si4])
    end

    it 'approved scope returns qualifying study involvements' do
      date = Date.today
      si1 = FactoryBot.create(:study_involvement)
      si2 = FactoryBot.create(:study_involvement)
      si3 = FactoryBot.create(:study_involvement)
      FactoryBot.create(:study_involvement_status, study_involvement: si2)
      FactoryBot.create(:study_involvement_status, study_involvement: si3).approve!
  ...
  describe 'active?' do
    it 'should be true if end date is today' do
      study_involvement = FactoryBot.create(:study_involvement, end_date: Date.today)
      expect(study_involvement).to be_active
    end

    it 'should be true if end date is in future' do
      study_involvement = FactoryBot.create(:study_involvement, end_date: Date.today + 10.days)
      expect(study_involvement).to be_active
    end
  ...
  describe 'original_release?' do
    ...
    it 'returns true for study_involvements with extended_release flag set to false' do
      study_involvement = FactoryBot.create(:study_involvement, end_date: Date.today, extended_release: false)
      expect(study_involvement).to be_original_release
    end

    it 'returns false for study_involvements with extended_release flag set to true' do
      study_involvement = FactoryBot.create(:study_involvement, end_date: Date.today, extended_release: true)
  ...
  describe 'getting study_involvement status' do
    ...
    it 'returns status state if status is not yet approved' do
      study_involvement         = FactoryBot.build(:study_involvement, start_date: date, end_date: date + 1.days)
      study_involvement_status  = FactoryBot.create(:study_involvement_status, study_involvement: study_involvement)
